<?php
	...
	if (file_get_contents("http://assets.artstation.mattfiler.co.uk/VIDEOS/" . $_GET['location'] . "/" . $_GET['video'] . ".csv") == "") {
		$raw_data = file_get_contents("http://assets.artstation.mattfiler.co.uk/VIDEOS/" . $_GET['location'] . "/" . $_GET['video'] . ".srt");
		if ($raw_data == "") {
			$raw_data = file_get_contents("http://assets.artstation.mattfiler.co.uk/VIDEOS/" . $_GET['location'] . "/" . $_GET['video'] . ".SRT");
		}
		$line_count = 0;
	...
	if ($json_data == array()) {
		$raw_data = file_get_contents("http://assets.artstation.mattfiler.co.uk/VIDEOS/" . $_GET['location'] . "/" . $_GET['video'] . ".csv");
		$line_count = 0;
		$previous_line = array();
		foreach(preg_split("/((\r?\n)|(\r\n?))/", $raw_data) as $line){
			if ($line_count > 0) {
